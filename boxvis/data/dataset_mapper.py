...
from .datasets.bvisd import COCO_TO_BVISD, YTVIS_2021_TO_BVISD, OVIS_TO_BVISD
...
class YTVISDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        if not is_tgt:
            ...
            if tgt_dataset_name.startswith("bvisd_train"):
                if src_dataset_name.startswith("ytvis_2021"):
                    src2tgt = YTVIS_2021_TO_BVISD
                elif src_dataset_name.startswith("ovis"):
                    src2tgt = OVIS_TO_BVISD
                else:
    ...
    @classmethod
    def from_config(cls, cfg, is_train: bool = True, is_tgt: bool = True, tgt_dataset_name=None):
...
class CocoClipDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        t = 0
        while t < self.sampling_frame_num:
...
def display_pseudo_clip_from_coco(images_list, file_name):
    output_path = '/data/code/BoxVIS_ICCV/output/bvisd/visual/pseudo_coco_clip/600_800_brightness/'
    import os
    os.makedirs(output_path, exist_ok=True)
    print(x.shape() for x in images_list)
