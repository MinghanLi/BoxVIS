...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        ...
        for d_i, dataset_name in enumerate(cfg.DATASETS.TRAIN):
            is_tgt = False if cfg.MODEL.BoxVIS.BVISD_ENABLED else d_i == len(cfg.DATASETS.TRAIN) - 1
            tgt_dataset_name = "bvisd_train" if cfg.MODEL.BoxVIS.BVISD_ENABLED else None
            if dataset_name.startswith('coco'):
                mappers.append(
                    CocoClipDatasetMapper(
                        cfg, is_train=True, is_tgt=is_tgt,
                        src_dataset_name=dataset_name,  tgt_dataset_name=tgt_dataset_name
                    )
                )
